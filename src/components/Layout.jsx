...
export const Layout = ({ dataRecieved, movieSelected }) => {
  ...
  useEffect(() => {
    currentChild === "Male Only"
      ...
      ? sortByGender("female")
      : currentChild === "Hermaphrodite Only"
      ? sortByGender("hermaphrodite")
      : sortByGender("all");
  }, [currentChild]);
  ...
  const displayCharacters = characters_context.map((item, index) => {
    return (
      <TableData 
        ...
        gender={
            item.gender === 'male' ? 'M' :
            item.gender === 'female' ? 'F' :
            item.gender === 'hermaphrodite' ? 'H' :
            item.gender 
        } 
    />
    );
      ...
      <section className="table">
        ...
        {characters_context.length > 0 && (
          <div className="selection">
            ...
            <button
              onClick={(e) => setCurrentChild("Hermaphrodite Only")}
              className={
                currentChild === "Hermaphrodite Only" ? "selected" : "non_selected"
              }
            >
              Hermaphrodite Only
            </button>
        ...
        {characters_context.length > 0 && (
            <>
                <h5 className="genderKey">Gender Key: male(M), female(F), hermaphrodite(H)</h5>
                <table>
                    <thead>
